...
target_ukb$father_stroke<-ifelse(rowSums(illness_father=="Stroke",na.rm=T)>0,1,0)
illness_mother<-allukb1[,which(grepl("f.20110.0.",colnames(allukb1)))]
target_ukb$mother_stroke<-ifelse(rowSums(illness_mother=="Stroke",na.rm=T)>0,1,0)
illness_silbings<-allukb1[,which(grepl("f.20111.0.",colnames(allukb1)))]
target_ukb$silbings_stroke<-ifelse(rowSums(illness_silbings=="Stroke",na.rm=T)>0,1,0)
...
#dbp<-allukb1[,c("f.4079.0.0","f.4079.0.1")]
#dbp<-rowMeans(dbp,na.rm = T)
...
target_ukb<-cbind(target_ukb,sbp)
colnames(target_ukb)[ncol(target_ukb)]<-"sbp"

# Data-Field 30870 Description: Triglycerides
# Data-Field 30710 Description: C-reactive protein
supp_ukb<-allukb1[,c("f.30870.0.0","f.30780.0.0","f.30710.0.0")]
names(supp_ukb)<-c("Trig","LDL","crp")
target_ukb<-cbind(target_ukb,supp_ukb)

target_ukb<-cbind(target_ukb,supp_ukb)
colnames(target_ukb)[ncol(target_ukb)]<-"LDL"
...
target_ukb$Y<-ifelse(rownames(target_ukb)%in%stroke_inc_eid,1,0)
target_ukb<-target_ukb[which(!rownames(target_ukb)%in%stroke_prv_eid),]
...
target_ukb$prv_cvd<-ifelse(rownames(target_ukb)%in%cvd_prv_eid,1,0)
...
target_ukb<-cbind(target_ukb,prsscore$p26248)
