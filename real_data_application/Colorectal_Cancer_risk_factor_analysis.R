...
index_riskf<-c("f.eid","f.31.0.0","f.21022.0.0","f.21001.0.0","f.50.0.0","f.20116.0.0","f.1558.0.0","f.20161.0.0")
names_riskf<-c("Y","sex","age","BMI","Height","SmokingStatus","AlcoholFreq","PackYrs")
...
target_ukb$father_Bowel_cancer<-ifelse(rowSums(illness_father=="Bowel cancer",na.rm=T)>0,1,0)
illness_mother<-allukb1[,which(grepl("f.20110.0.",colnames(allukb1)))]
target_ukb$mother_Bowel_cancer<-ifelse(rowSums(illness_mother=="Bowel cancer",na.rm=T)>0,1,0)
illness_silbings<-allukb1[,which(grepl("f.20111.0.",colnames(allukb1)))]
target_ukb$silbings_Bowel_cancer<-ifelse(rowSums(illness_silbings=="Bowel cancer",na.rm=T)>0,1,0)

supp_ukb<-allukb1[,c("f.1349.0.0")]
names(supp_ukb)<-c("processedmeat")
target_ukb<-cbind(target_ukb,supp_ukb)
colnames(target_ukb)[ncol(target_ukb)]<-"processedmeat"

library(readr)
extra<-read_tsv("/dcs04/nilanjan/data/ukb_protein_arking/Risk_factors_060223_participant.tsv")
extra<-data.frame(extra)
extra<-extra[match(allukb1$f.eid,extra$eid),]
physical<-extra[,"p22040_i0"]
target_ukb<-cbind(target_ukb,physical)
colnames(target_ukb)[ncol(target_ukb)]<-"physical"

# case definition 

target_ukb$Y<-ifelse(rownames(target_ukb)%in%colon_inc_eid,1,0)
target_ukb<-target_ukb[which(!rownames(target_ukb)%in%colon_prv_eid),]

target_ukb$prv_t2d<-ifelse(rownames(target_ukb)%in%t2d_prv_eid,1,0)
target_ukb$prv_crohn<-ifelse(rownames(target_ukb)%in%crohn_prv_eid,1,0)
target_ukb$prv_polyps<-ifelse(rownames(target_ukb)%in%polyps_prv_eid,1,0)

## filtering or imputation 
target_ukb<-target_ukb[rowSums(is.na(target_ukb[,c("sex","age","BMI","Height","SmokingStatus","AlcoholFreq")]))==0,]
...
target_ukb<-target_ukb[!target_ukb$processedmeat%in%c("Prefer not to answer","Do not know"),]
allmeat<-target_ukb$processedmeat
allmeat2<-rep(NA,length(allmeat))
allmeat2[allmeat == "Never"]<-0
allmeat2[allmeat == "Less than once a week"]<-1
allmeat2[allmeat == "Once a week"]<-2
allmeat2[allmeat == "2-4 times a week"]<-3
allmeat2[allmeat == "5-6 times a week"]<-4
allmeat2[allmeat == "Once or more daily"]<-5
target_ukb$processedmeat<-ifelse(allmeat2==1,1,0)
target_ukb$processedmeat2<-ifelse(allmeat2==2,1,0)
target_ukb$processedmeat3<-ifelse(allmeat2==3,1,0)
target_ukb$processedmeat4<-ifelse(allmeat2==4,1,0)
target_ukb$processedmeat5<-ifelse(allmeat2==5,1,0)
...
target_ukb<-cbind(target_ukb,prsscore$p26218)
