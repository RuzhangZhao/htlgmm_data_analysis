...
pZ = 40
pW = 1500
nlist = c(400,600,800,1000,1200,1500,2000,2500,3000,4000,5000,6000)
n = nlist[nid]
print(paste0("sample:",n))
savepath="htlgmm_simulate/logistic_pW_1500/" #self-defined save path
pacman::p_load(htlgmm,expm,magic,glmnet,cluster,MASS,locfit,corpcor,caret,pROC,mvtnorm)
if(0){
...
if(pZ==10){
    ...
    coefZ[Z_nonnull_index]=0.3/2
}else if(pZ == 40){
    Z_nonnull_index=c(1,2,3,14,15,26,27,28,39,40)
    coefZ[Z_nonnull_index]=0.3/2*1.28
}
W_nonnull_index = c(1,2,3,7,11,16,21,24,26,40,54,57,59,78,80)
coefW[W_nonnull_index]=0.25/2
coefZW = c(coefZ,coefW)

correct_pos = c(1,1+c(Z_nonnull_index,(pZ+W_nonnull_index)))
correct_Zpos = 1+Z_nonnull_index
correct_Wpos = 1+pZ+W_nonnull_index
ZWlinklist=list()
ZWlinklist[[10]]=c(8,9)
ZWlinklist[[8]]=c(1)
ZWlinklist[[6]]=c(2)
...
simulate_Xy=function(n,pZ=pZ,pW=pW,coefs=coefZW,intercept=intercept,
    ...
    X=scale(X)
    y=rbinom(n,size = 1,prob=expit(X%*%coefs+intercept))
    list("X"=X,"y"=y)
}

summary_stat=function(Z,y){
    study_info=list()
    reducedglm=glm(y~.,data = data.frame(y,Z),family = "binomial")
    study.m = list(Coeff=reducedglm$coefficients[-1],
                   Covariance=vcov(reducedglm)[-1,-1],Sample_size=nrow(Z))
    study_info[[1]] = study.m
    return(list("glm"=reducedglm,"study_info"=study_info))
}


adalasso_inference<-function(res_ada,X_main,y_main){
    beta_ada<-c(coef(res_ada,s="lambda.min")[1],coef(res_ada,s="lambda.min")[-1])
    diag_term<-c(expit(cbind(1,X_main)%*%beta_ada)*(1-expit(cbind(1,X_main)%*%beta_ada)))
    Sigsum_scaled<-t(cbind(1,X_main))%*%(diag_term*cbind(1,X_main))
...
if(!file.exists(paste0(savepath,"logistic_test_pZ_",pZ,".rds"))){
    ...
    y0=c(Xy0$y)
    RC(y0,X0%*%coefZW)
    saveRDS(Xy0,paste0(savepath,"logistic_test_pZ_",pZ,".rds"))
}else{
    Xy0=readRDS(paste0(savepath,"logistic_test_pZ_",pZ,".rds"))
    X0=Xy0$X
    y0=c(Xy0$y)
    #RC(y0,X0%*%coefZW)
}
# adalasso with lasso initial
savefilename = paste0(savepath,"logistic_n_",n,"_pZ_",pZ,"_r_10_seed",seed.use,".rds")
savefilenamea = paste0(savepath,"logistic_n_",n,"_pZ_",pZ,"_r_10ada_seed",seed.use,".rds")
...
for(i in (length(savefilelist)+1):100){
    ...
    res_lasso=cv.glmnet(x = X_main,y = y_main,family = "binomial",type.measure = "auc")
    res_ridge=cv.glmnet(x = X_main,y = y_main,alpha=0,family = "binomial",type.measure = "auc")
    
    beta_lasso=c(coef(res_lasso,s="lambda.min")[-1])
    beta_ridge=c(coef(res_ridge,s="lambda.min")[-1])
    
    www=1/abs(c(coef(res_ridge,s="lambda.min")[-1]))^(1/2)
    
    res_ada=cv.glmnet(x = X_main,y = y_main,penalty.factor = www,family = "binomial",,type.measure = "auc")
    ...
    RC_main_ridge=RC(y0,X0%*%beta_ridge)
    
    RC_main_adalasso=RC(y0,X0%*%coef(res_ada,s="lambda.min")[-1])
    runhtlgmm=function(y_main,X_main,study_info,res_lasso,penalty_type,www=NULL){
        ...
        tryCatch(
            {
                ...
                res_htlgmm_lasso_tuneC_mshrink=cv.htlgmm(y = y_main,Z = Z_main,W = W_main,
                                                       ...
                                                       ext_study_info = study_info,family = "binomial",
                                                       use_sparseC = F,
                                                       tune_weight = T,
                ...
                res_htlgmm_lasso_owGMM=cv.htlgmm(y = y_main,Z = Z_main,W = W_main,
                                                  ...
                                                  weight_adaptivelasso = www,
                                                  inference = doinference,
                                                  ext_study_info = study_info,family = "binomial",use_sparseC = F)
                
                weightC_mshrink = res_htlgmm_lasso_tuneC_mshrink$weight_min
                weightC_ridge = res_htlgmm_lasso_tuneC_ridge$weight_min
                auc_tuneC_mshrink=RC(y0,X0%*%res_htlgmm_lasso_tuneC_mshrink$beta[-1])
                auc_tuneC_ridge=RC(y0,X0%*%res_htlgmm_lasso_tuneC_ridge$beta[-1])
                auc_owGMM=RC(y0,X0%*%res_htlgmm_lasso_owGMM$beta[-1])
                
                RC_list=c(mean(y_main),RC_true_model,RC_true_Z,RC_true_W,RC_ext,RC_main_lasso,RC_main_ridge,RC_main_adalasso,
                          auc_tuneC_mshrink,weightC_mshrink,
                          auc_tuneC_ridge,weightC_ridge,
                          auc_owGMM)
                
                names(RC_list)=c("case_ratio","true_model","true_Z","true_W","external_glm","main_lasso","main_ridge","main_ada",
                                 "tuneC_mshrink","weightC_mshrink",
                                 "tuneC_ridge","weightC_ridge",
                                 "owGMM")
                ...
                if(penalty_type == "adaptivelasso"){
                    ...
                    colnames(res11)<-c('ada','mshrink',
                                       'ridge','owGMM')
                    rownames(res11)<-c('powerW','fdr',"cv")
                    print(res11)
                }
        ...
        resres<-list("RC"=RC_list)
        if(penalty_type == "adaptivelasso"){
            resres<-list("RC"=RC_list,"inference"=inf_res)
        }
        resres
    }
    savefilelist[[i]]=runhtlgmm(y_main,X_main,study_info10,res_lasso,"lasso")
    savefilelista[[i]]=runhtlgmm(y_main,X_main,study_info10,res_lasso,"adaptivelasso",www)
    if (i %% 5 == 0){
